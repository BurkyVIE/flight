...
topapts <- toptrips %>%
  ...
  enframe(name = NULL) %>%
  rename(IATA = value) %>%
  left_join(airports %>% select(IATA,
                                Long,
                                Lat),
            by = c("IATA" = "IATA"))
...
paths <- toptrips %>%
  select(Trip, Path) %>% unnest(cols = Path)
...
p1 <- ggplot(data = toptrips) +
  ...
  geom_text(mapping = aes(x = 0, y = (topn:1) - 0.5, label = paste0("Trip ",
                                                                    Trip,
                                                                    " (",
                                                                    format(Distance %>% round(0), big.mark = ","),
                                                                    " km)\n",
                                                                    Route),
...
# PLOT 2
p2 <- ggplot(data = toptrips, mapping = aes(x = reorder(as.character(Trip), Distance, function(x) -x), y = Distance, fill = as.character(Trip))) +
...
# Flugpfade
p3 <- p3 +
  geom_path(data = paths, mapping = aes(x = lon, y = lat, group = Trip, color = as.character(Trip)), linewidth = 1.5, show.legend = FALSE) +
  scale_color_manual(values = color)
...
#windows(18, 8)
pdf("top trips.pdf", width = 18, height = 8)
gridExtra::grid.arrange(p1,p2,p3,layout_matrix=matrix(c(1,3,3,3,3,2,3,3,3,3),nrow=2,byrow=T),
                        top=grid::textGrob(paste("Top", topn, "Trips"),gp=grid::gpar(fontsize=17)))
dev.off()
