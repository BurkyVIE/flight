...
paths <- myroutes %>% select(Route, Path) %>% unnest(cols = Path)
p1 <- p1 + geom_path(data = paths, mapping = aes(x = lon, y = lat, group = Route), color = colo[3])
...
# PLOT 2
p2 <- ggplot(data = myflights, mapping = aes(x = Distance, fill = Relation)) +
